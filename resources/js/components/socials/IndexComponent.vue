                        ...
                        <div class="kt-portlet__body">
                            ...
                            <table class="table table-striped table-bordered dataTable table-checkable table-sorting ">
                                ...
                                <tr :data-id="social.id"
                                    v-for="(social, index) in allReturnSocialsIndex.socials"
                                    :key="index">
                                    <td>{{ sorting ? allReturnSocialsIndex.offset+index+1 :
                                        (allReturnSocialsIndex.totalCount - allReturnSocialsIndex.offset)- index }}
                                        <img width="80px" :src="'/uploads/socials/'+social.icon">
                                    </td>
                                    <td>{{ social.name }}</td>
                                    <td>{{ social.url }}</td>
                                    <td width="120px">
                                        <router-link :to="'/vagart-cms/socials/'+social.id+'/edit'"
                            ...
                            <div v-if="allReturnSocialsIndex.totalCount > 10 ? true : false"
                                 class="kt-pagination  kt-pagination--brand">
                                <paginate
                                    v-if="allReturnSocialsIndex.count"
                                    :page-count="allReturnSocialsIndex.count"
                                    :page-range="5"
                                    :margin-pages="1"
                                ...
                                <div class="kt-pagination__toolbar">
                                    <select v-if="allReturnSocialsIndex.count" @change="changeData" v-model="pageCount"
                                    ...
                                    <span class="pagination__desc">
                                        Показано {{ allReturnSocialsIndex.displaying }} из  {{ allReturnSocialsIndex.totalCount }}
                                    </span>
                                </div>
                            </div>
...
<script>
    ...
    export default {
        ...
        computed: mapGetters([
            "allReturnSocialsIndex"
        ]),

        methods: {
            sort() {
                this.sorting = !this.sorting;
                this.$store.commit('changeSocialsStates', {
                    pageNum: this.page,
                    pageCount: this.pageCount,
                    sorting: this.sorting,
                });
                this.$store.dispatch('returnSocialsIndex');
            },
