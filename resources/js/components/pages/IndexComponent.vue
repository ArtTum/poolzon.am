                    ...
                    <div class="kt-portlet kt-portlet--mobile">
                        ...
                        <div class="kt-portlet__body">
                            ...
                            <table class="table table-striped table-bordered dataTable table-checkable table-sorting ">
                                ...
                                <draggable v-model="list" tag="tbody" :component-data="getComponentData()"
                                           :disabled="!enabled" :options="{animation:500}">
                                    <tr ref="sort" :data-id="page.id"
                                        v-for="(page, index) in allReturnPagesIndex.pages" :key="index">
                                        <td align="center" style="cursor: grab; font-size: 20px" scope="row"><i
                                            class="la la-align-justify"></i></td>
                                        <td align="">{{ (currentSortDir=='asc') ?
                                            allReturnPagesIndex.offset+index+1 :
                                            (allReturnPagesIndex.totalCount - allReturnPagesIndex.offset)-
                                            index }}
                                        </td>
                                        <td>{{ page.page_name }}</td>
                                        <td>{{ page.alias }}</td>
                                        <td>{{ page.meta_title }}</td>
                                        <td>{{ page.meta_keys }}</td>
                                        <td>{{ page.meta_description }}</td>
                                        <td>{{ !page.page_status ? 'Пассивный' : 'Активный'}}</td>
                                        <td>
                                            <a @click="alertDisplay(page.page_status, page.id)" style="cursor: pointer"
                                               class="btn btn-sm btn-clean btn-icon btn-icon-md"
                                               title="Change Status"><i
                                                class="la la-lightbulb-o"></i>
                                            </a>
                                            <router-link :to="'/vagart-cms/pages/'+page.id+'/edit'"
                                                         class="btn btn-sm btn-clean btn-icon btn-icon-md"
                                                         title="Edit Ad"><i
                                                class="la la-edit"></i>
                                            </router-link>
                            ...
                            <div v-if="allReturnPagesIndex.totalCount > 10 ? true : false"
                                 class="kt-pagination  kt-pagination--brand">
                                <paginate
                                    v-if="allReturnPagesIndex.count"
                                    :page-count="allReturnPagesIndex.count"
                                    :page-range="5"
                                    :margin-pages="1"
                                ...
                                <div class="kt-pagination__toolbar">
                                    <select v-if="allReturnPagesIndex.count" @change="changeData"
                                    ...
                                    <span class="pagination__desc">
                                        Показано {{ allReturnPagesIndex.displaying }} из  {{ allReturnPagesIndex.totalCount }}
                                    </span>
                                </div>
                            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            changeData: function () {
                this.$store.commit('changePages', {
                    pageNum: this.page,
                    pageCount: this.pageCount,
                    sort_number: this.currentSortDir
                });
                this.$store.dispatch('returnPagesIndex');
            },
            handleChange() {
            ...
            alertDisplay(status, id) {
                ...
                this.$swal({
                    ...
                    if (result.value) {
                        this.$store.commit('changePageId', {
                            id
                        });
                        if (this.$store.dispatch('changePageStatus')) {
                            this.$store.dispatch('returnPagesIndex');
                        }
